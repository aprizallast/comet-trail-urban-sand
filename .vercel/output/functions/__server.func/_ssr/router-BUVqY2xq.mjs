...
//#region node_modules/.nitro/vite/services/ssr/assets/router-BUVqY2xq.js
var router_BUVqY2xq_exports = /* @__PURE__ */ __exportAll({ getRouter: () => getRouter });
...
var styles_default = "/assets/styles-DExmKQGy.css";
...
var Route$9 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "Live launchpad radar for brew.family tokens on BNB Chain — tape, dev clusters, and a tactical copilot."
			},
			{
				name: "theme-color",
				content: "#0c0806"
...
var $$splitComponentImporter = () => import("./routes-Cm45Yf6Y.mjs");
var Route$8 = createFileRoute("/")({ component: lazyRouteComponent($$splitComponentImporter, "component") });
...
function mapLaunches(launches) {
	...
	const tokens = launches.map((l, idx) => {
		...
		return {
			...
			priceUsd: marketCap > 0 ? marketCap / 1e9 : 0,
			priceChange24h: 0,
			volume24h,
			liquidityUsd: 0,
			marketCap,
...
var Route$6 = createFileRoute("/api/dex")({ server: { handlers: { GET: async ({ request }) => {
	const addrs = (new URL(request.url).searchParams.get("addrs") || "").split(",").map((a) => a.trim().toLowerCase()).filter((a) => /^0x[a-f0-9]{40}$/.test(a)).slice(0, 30);
	if (addrs.length === 0) return Response.json([]);
	const key = addrs.join(",");
	const hit = cache.get(key);
	if (hit && Date.now() - hit.at < 2e4) return Response.json(hit.body);
	try {
...
var Route$5 = createFileRoute("/api/inspect")({ server: { handlers: { GET: async ({ request }) => {
	...
	const [dex, sec] = await Promise.allSettled([fetch(`https://api.dexscreener.com/tokens/v1/bsc/${address}`).then((r) => r.ok ? r.json() : null), fetch(`https://api.gopluslabs.io/api/v1/token_security/56?contract_addresses=${address}`).then((r) => r.ok ? r.json() : null)]);
...
//#endregion
export { getRouter, router_BUVqY2xq_exports as t };
